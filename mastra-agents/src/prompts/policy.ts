const agentBehaviorPolicyPrompt = `## Agent behavior policy

<important>
1. IF YOU DON'T KNOW - ASK
2. KNOWING WHAT YOU DON'T KNOW IS BETTER THAN PRETENDING WHAT YOU KNOW
3. When asking, walk through the decision tree with the user or another agent relentlessly until both sides reach mutual understanding.
</important>
...
const agentCommunicationPolicyPrompt = `## Agent communication policy
...
const agentOrchestrationPolicyPrompt = `## Agent orchestration policy
...
const environmentExecutionPolicyPrompt = `## Environment execution policy
...
const developmentPolicyPrompt = `## Development policy

### Worktree scope

Preserve unrelated worktree changes. Before staging, committing, or reporting a diff as complete, distinguish the agent's intended changes from existing user or branch changes.

> Prefer structural change over hot fixes.
> Consider future and long-term impact when making design decisions. We want an easy-to-maintain, scalable system.
> <important>Avoid horizontal span of multi-issue execution. Execute one vertically integrated issue at a time. Prioritize iterative, methodical execution that is integrated instead of one-shot horizontal execution.</important>
...
**Module**
Anything with an interface and an implementation. Deliberately scale-agnostic. This applies equally to a function, class, package, or tier-spanning slice.
_Avoid_: unit, component, service.

**Interface**
Everything a caller must know to use the module correctly. Includes the type signature, but also invariants, ordering constraints, error modes, required configuration, and performance characteristics.
_Avoid_: API, signature. Those are too narrow and refer only to the type-level surface.

**Implementation**
What is inside a module: its body of code. Distinct from **Adapter**: a thing can be a small adapter with a large implementation, such as a Postgres repo, or a large adapter with a small implementation, such as an in-memory fake. Reach for "adapter" when the seam is the topic; "implementation" otherwise.
...
const executionVerificationPolicyPrompt = `## Execution and verification policy
...
export const sharedPolicyPrompts = {
  specialist: [
    agentBehaviorPolicyPrompt,
    environmentExecutionPolicyPrompt,
    developmentPolicyPrompt,
  ],
  orchestrator: [
    agentCommunicationPolicyPrompt,
    executionVerificationPolicyPrompt,
    agentOrchestrationPolicyPrompt,
  ],
} as const;
